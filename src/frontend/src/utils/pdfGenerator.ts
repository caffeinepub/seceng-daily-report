export interface ReportData {
  ...
  personnel: Array<{ leadTechnician: string; assistantTechnicians: string }>;
...
function getJsPDF(): new (...args: unknown[]) => JsPDFInstance {
  ...
  throw new Error(
    "jsPDF not loaded. Make sure the CDN script is included in index.html.",
  );
}
...
function drawSectionHeader(
  ...
  let y = checkPageBreak(doc, yPos, 12);
...
function drawField(
  ...
  let y = checkPageBreak(doc, yPos, 8);
  doc.setFontSize(7.5);
  doc.setFont("helvetica", "bold");
...
export async function generatePDF(data: ReportData): Promise<void> {
  ...
  try {
    doc.addImage(
      "/assets/generated/seceng-logo.dim_256x256.png",
      "PNG",
      MARGIN,
      4,
      18,
  ...
  for (const [l1, v1, l2, v2] of infoRows) {
    ...
    const endY2 = l2 ? drawField(doc, startY, l2, v2, col2x, colW) : startY;
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...C.midGray);
  doc.text("LEAD TECHNICIAN", MARGIN + 2, y + 5);
  doc.text("ASSISTANT TECHNICIANS", MARGIN + 95, y + 5);
  y += 9;

  for (const person of data.personnel) {
    if (!person.leadTechnician && !person.assistantTechnicians) continue;
    y = checkPageBreak(doc, y, 7);
    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...C.black);
    doc.text(person.leadTechnician || "—", MARGIN + 2, y + 4);
    doc.text(person.assistantTechnicians || "—", MARGIN + 95, y + 4);
    y += 7;
    doc.setDrawColor(...C.lightGray);
  ...
  if (hasSitePhotos) {
    ...
    for (let i = 0; i < data.sitePhotos!.length; i++) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...C.midGray);
      doc.text(`PHOTO ${i + 1}`, photoX + thumbW / 2, photoY + thumbH + 5, {
        align: "center",
      });
    }
  } else {
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...C.midGray);
  doc.text("Authorized Signature", sigColX, sigY);

  // Advance y past both columns
  const photosFinalY = hasSitePhotos
    ? contentStartY +
      (Math.ceil(data.sitePhotos!.length / 2) - 1) * (thumbH + 10) +
      thumbH +
      12
    : contentStartY + 20;
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(`Page ${p} of ${totalPages}`, PAGE_W - MARGIN, PAGE_H - 4, {
      align: "right",
    });
  }

  const filename = `DFR_${data.projectNumber || "report"}.pdf`;
  doc.save(filename);
}
